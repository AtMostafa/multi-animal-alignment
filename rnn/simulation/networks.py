...
class RNN(nn.Module):
    ...
    def force_training(self, X, P, output_weights_inv, target):
        ...
        out = torch.zeros(tsteps, batch_size, self.n_outputs).type(self.dtype) #initial output
...
class RNN_muscle(nn.Module):
    '''
        Class for RNN to model neural dynamics and motor output
    '''
    def __init__(self, n_inputs, n_outputs, n_neurons, alpha, dtype, noise=None, p_recurrent = 1.0, n_muscles =50):
        super(RNN_muscle, self).__init__()
        ...
        #muscle layer
        self.muscle = nn.Linear(n_neurons, n_muscles)

        #nonlinear output layer
        self.output = nn.Linear(2*n_muscles, n_outputs)
    ...
    def forward(self, X):
        ...
        out = torch.zeros(tsteps, self.batch_size, self.n_outputs).type(self.dtype) #initial output
        # muscle = torch.zeros(tsteps, self.batch_size, self.n_muscles).type(self.dtype) #initial output
        hiddenl1 = torch.zeros(tsteps, self.batch_size, self.n_neurons).type(self.dtype)
        ...
        for j in range(tsteps): 
            ...
            hiddenl1[j] = r1
            musc = self.muscle(r1)
            # temp = torch.cat((musc, torch.square(musc)), 2)
            temp = torch.cat((torch.sin(musc), torch.cos(musc)), 2)
            out[j] = self.output(temp)
